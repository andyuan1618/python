...
#发送一次网络请求
def getResponse(target_url, proxies, headers, datas, method='POST'):
    s = requests.Session()
    ...
    try:
        logging.getLogger().info(method)
        if method == 'POST':
            r = s.post(target_url, headers=headers, proxies=proxies, timeout=15, verify=False, data=datas)
            logging.getLogger().info(r.text)
        elif method == 'GET':
            r = s.get(target_url, headers=headers, proxies=proxies, timeout=15, verify=False)
            logging.getLogger().info(r.text)
        else:
            logging.getLogger().info('unknown method')
            return {'code':-4, 'content':u'unknown method'}
...
#获取省份名称和代码hash表
def getProvinceMap():
    res = db.sql_fetch_rows("SELECT `code`, `name` FROM common_area_code WHERE `type` = 3 order by `code`")
    map = {}
    for item in res:
        areacode = str(item[0])
        map[areacode] = item[1]
...
def getSearchList(proxies, company_name, province_name):
    target_url = "http://weiche.co/ECIUI/Search"
    headers = {
        "Host":"weiche.co",
        "Accept":"*/*",
        ...
        "Origin":"http://weiche.co",
        "User-Agent":"Mozilla/5.0 (iPhone; CPU iPhone OS 9_3 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Mobile/13E234 MicroMessenger/6.3.22 NetType/WIFI Language/zh_CN",
        "Cookie":"Hm_lpvt_40223ef4fa055b667e18ea8afa13450b=1469079559; Hm_lvt_40223ef4fa055b6    67e18ea8afa13450b=1468398548,1468834657,1468940657,1469070224; ASP.NET_SessionId=m2fkrb21hfwr4agtziidhmz4; CNZZDATA1256425432=555819473-1467635477-%7C1469075712; SERVERID=82c47d0c5de873    da5a3a2a53e17ec318|1469079559|1469079554; lastProvice=AH;",
        "Referer":"",
    }
    datas = {
        'province' : province_name,
        'ticket':'2DB08FF6E9EDA300C5BF270BE5FA8BD6',
        'key':quote(company_name.encode('utf-8')),
    }
    headers['Referer'] = "http://weiche.co/ECIUI/Search?provice=" + province_name
    resp = getResponse(target_url=target_url, proxies=proxies, headers=headers, datas=datas, method='POST')
...
def getSearchKey(start, limit):
    res = db.sql_fetch_rows("SELECT `id`, `area_code`, `name` FROM `common_company_name` WHERE `id` > %d AND `id` < %d AND `qichacha_weixin_status` = 0" % (start, start + limit))
    return res



def work():
    site = db.sql_fetch_one_cell("SELECT `value` FROM `common_config` WHERE `key` = 'qichacha_weixin_site'")
    while True:
        common_config = db.sql_fetch_one_cell("SELECT `value` FROM `common_config` WHERE `key` = 'qichacha_weixin_crawler'")
        ...
        db.sql_update("UPDATE `common_config` SET `value` = '%s' WHERE `key` = 'qichacha_weixin_crawler'" % (common_config))
        #break
F
