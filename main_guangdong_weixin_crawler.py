...
def work():
    ...
    while True:
        ...
        tasks = db.sql_fetch_rows("SELECT `id`, `name`, `guangdong_weixin_status` FROM `common_company_name` WHERE `guangdong_weixin_status` = 0 AND `area_code` = 440000 LIMIT %d "% (limit))
        if tasks != None:
            for task in tasks:
